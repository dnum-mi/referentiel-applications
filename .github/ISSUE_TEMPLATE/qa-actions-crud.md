---
name: "🧪 QA — Actions CRUD de base"
about: "Campagne de non-régression des actions CRUD de base (fiche application) pour une version"
title: "[QA][vX.Y.Z] Actions CRUD de base"
labels: ["qa", "non-regression", "qa:actions-crud"]
---

## Campagne

- **Version testée** : `vX.Y.Z`
- **Environnement** : (prod / recette / local)
- **Navigateur** :
- **Testeur** :
- **Date** :

> Protocole de référence : [`qa/protocoles/actions-crud.md`](../blob/main/qa/protocoles/actions-crud.md).
> Cocher chaque étape validée et **glisser une capture** dans la zone `📎`. Tout échec → ouvrir un
> 🐛 (template _Rapport de bug_) et lier l'issue ici, laisser la case décochée.

## Récapitulatif

| Total | ✅ Passés | ❌ Échecs | ⏭️ Non testés |
| ----- | --------- | --------- | ------------- |
| 16    |           |           |               |

## Protocole

<!-- qa-steps -->

## Bugs liés

-

## Verdict

- [ ] `qa:pass` — aucune régression bloquante
- [ ] `qa:fail` — régression(s) constatée(s) (voir bugs liés)
