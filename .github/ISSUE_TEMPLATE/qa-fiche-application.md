---
name: "🧪 QA — Fiche application"
about: "Campagne de non-régression de la fiche application pour une version"
title: "[QA][vX.Y.Z] Fiche application"
labels: ["qa", "non-regression", "qa:fiche"]
---

## Campagne

- **Version testée** : `vX.Y.Z`
- **Environnement** : (prod / recette / local)
- **Navigateur** :
- **Testeur** :
- **Date** :

> Protocole de référence : [`qa/protocoles/fiche-application.md`](../blob/main/qa/protocoles/fiche-application.md).
> Cocher chaque étape validée et **glisser une capture** dans la zone `📎`. Tout échec → ouvrir un
> 🐛 (template _Rapport de bug_) et lier l'issue ici, laisser la case décochée.

## Récapitulatif

| Total | ✅ Passés | ❌ Échecs | ⏭️ Non testés |
| ----- | --------- | --------- | ------------- |
| 16    |           |           |               |

## Protocole

<!-- qa-steps -->

## Bugs liés

-

## Verdict

- [ ] `qa:pass` — aucune régression bloquante
- [ ] `qa:fail` — régression(s) constatée(s) (voir bugs liés)
