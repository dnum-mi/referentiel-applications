...
const compliances = ref<Compliance | null>(null);
...
const fetchQuality = async () => {
  ...
  try {
    ...
    try {
      ...
      // If no compliance exists, that's fine - set to null
      compliances.value = null;
    }
  } catch (error) {
  ...
  <div v-else class="fr-grid-row fr-grid-row--gutters">
    ...
    <div class="fr-col-12 fr-col-md-4">
      ...
      <DsfrHighlight :color="getComplianceColor('DIMA')" :small="true"
        >DIMA : {{ getComplianceStatus("DIMA") }}</DsfrHighlight
      >
      <DsfrHighlight :color="getComplianceColor('PDMA')" :small="true"
        >PDMA : {{ getComplianceStatus("PDMA") }}</DsfrHighlight
      >
      ...
      <DsfrHighlight :color="getComplianceColor('RGAA')" :small="true"
        >RGAA : {{ getComplianceStatus("RGAA") }}</DsfrHighlight
      >
      <DsfrHighlight :color="getComplianceColor('DSFR')" :small="true"
        >DSFR : {{ getComplianceStatus("DSFR") }}</DsfrHighlight
      >
      <DsfrHighlight :color="getComplianceColor('RGPD')" :small="true"
        >RGPD : {{ getComplianceStatus("RGPD") }}</DsfrHighlight
      >
    </div>
  </div>
