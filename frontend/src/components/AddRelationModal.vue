...
const submitRelation = async () => {
  ...
  try {
    await Applications.addRelationInApplication(applicationSourceId, selectedApplication.value.id, relationType.value);
    emit("add-relation", {
  ...
  <DsfrModal :opened="props.opened" :title="props.title" @close="closeModal">
    <template #default>
      ...
      <ul v-if="suggestions.length" class="suggestions-list">
        <li v-for="app in suggestions" :key="app.id" @click="selectApplication(app)" class="suggestion-item">
          {{ app.label }}
        </li>
      </ul>
    </template>
