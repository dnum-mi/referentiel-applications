...
- `isDefault`
  ...
  > Un seul type d'acteur porte `isDefault: true` : garanti en base par l'index unique
  > PARTIEL `ActorType_single_default_idx` (migration 20260904100000, #2510) — Prisma ne sait
  > pas l'exprimer ici, ne pas le supprimer lors d'un `migrate diff`.
