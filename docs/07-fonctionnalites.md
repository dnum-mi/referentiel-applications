...
**Accès incomplet — alerte et contact admin (#2593).** Quand l'utilisateur n'a pas la lecture complète de la fiche (typiquement un admin scopé hors périmètre de l'application), `ApplicationPage.vue` affiche une alerte d'avertissement (fermable pour la session) l'invitant à s'ajouter comme acteur ou à contacter un administrateur, avec un lien `mailto:` résolu via `GET /applications/:applicationId/contact-admin`. Logique de résolution (admin local du périmètre le plus proche → admin global → adresse support statique) détaillée dans [Permissions & sécurité](./06-permissions-et-securite.md), §7.4.
...
**Administrateur à contacter dans le profil.** L'onglet « Mes informations » affiche, pour tout utilisateur sauf un administrateur global, la ligne « Administrateur » : lien `mailto:` vers l'admin scopé le plus proche de l'organisation de l'utilisateur (pour `A/B/C` : admin `A/B/C`, sinon `A/B`, sinon `A` ; un admin scopé ne se voit jamais lui-même), sinon l'admin global, sinon l'adresse support (même règle que le contact admin d'une fiche, cf. [Permissions & sécurité](./06-permissions-et-securite.md) §7.4).
...
**Applications supprimées.** La vue transverse et son total, les alertes in-app et le recalcul planifié ignorent les technologies d'une application au statut « supprimée » (#2515, `ACTIVE_APPLICATION_WHERE` dans `backend/src/technology/utils/eol-status.ts`). La recherche du catalogue, elle, n'applique aucune exclusion serveur : elle repose sur le filtre de statut par défaut du front (`DEFAULT_FILTERS.currentStatus__in`), et l'utilisateur peut afficher les fiches supprimées en cochant « Supprimée ». La fiche elle-même reste consultable.
...
Le contenu principal affiche un écran **« Authentification forte requise »** (`WeakAuthBanner.vue`), avec une explication adaptée si le fournisseur n'a pas transmis le mode. La recherche, la navigation applicative, le profil et les notifications ne sont pas affichés. Un lien d'aide est proposé s'il est configuré.

Le bouton **« Se reconnecter »** force une nouvelle authentification et conserve la destination demandée. Si le fournisseur renvoie encore une session faible, le bouton propose **« Se déconnecter puis se reconnecter »**. Si ce second parcours échoue également, l'écran oriente vers le support. La bascule est conservée après rechargement. Le frontend attend la confirmation du backend avant de réafficher les données.

En `observe`, le niveau est visible dans le profil sans effet sur l'accès. L'activation de `enforce` reste un acte d'exploitation après qualification des parcours réels ; les fixtures locales ne prouvent pas le comportement de Passage2.
...
| Profil – administrateur à contacter | `UserInfoTab.vue` · `GET /users/me/contact-admin`                               | utilisateur connecté (ligne masquée pour un admin global)                          |
...
| Niveau d'authentification (#1985)   | `WeakAuthBanner.vue` · `backend/src/auth-level`                                 | en mode `enforce`, tout accès au référentiel exige une session forte               |
