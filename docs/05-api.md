...
- **Token de service pour une application tierce (#1988)** — les **deux** en-têtes `x-refapp-token: <token de service>` et `Authorization: Bearer <access token utilisateur>` sont obligatoires. Le compte humain doit déjà exister dans RefApp ; aucun compte n'est créé via ce canal. Le JWT est vérifié comme pour une connexion directe, y compris le niveau d'authentification lorsque `AUTH_LEVEL_MODE=enforce`. Les droits effectifs sont l'intersection de ceux de l'utilisateur et du service. L'identité courante et les écritures restent attribuées à l'humain. Aucun repli sur le JWT seul n'est permis lorsqu'un token API est fourni mais invalide.

**Compatibilité :** un token de service seul répond désormais **401**, y compris pour les tokens existants. Avant déploiement, les intégrations tierces doivent transmettre l'access token de leur utilisateur, accepté par le fournisseur OIDC configuré dans RefApp. Les traitements machine autonomes sans utilisateur ne sont pas couverts par ce contrat. Les tokens personnels ne doivent pas servir de compte partagé pour contourner ce contrôle.

Exemple d'appel tiers :

```http
GET /api/v2/applications HTTP/1.1
x-refapp-token: <token de service>
Authorization: Bearer <access token de l'utilisateur>
```

Un token inconnu, expiré ou révoqué, un JWT absent ou invalide, ou un utilisateur inconnu donnent **401**. Un compte bloqué, un niveau d'authentification insuffisant en mode `enforce`, des périmètres disjoints ou une action non autorisée donnent **403**. Les [règles de délégation](./06-permissions-et-securite.md#15-appels-via-un-système-tiers-1988) détaillent les plafonds et les restrictions sur les secrets.
